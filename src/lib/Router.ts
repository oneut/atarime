...
export class Router {
    ...
    route(path: string, routeClass: PageClass, name?: string) {
        this.connector.getRouteMatcher().addRoute(path, Promise.resolve(routeClass), name);
    }
    ...
    runWitInitialProps(initialProps: any, callback: (Root: React.FunctionComponent<{}>) => void) {
        const runner = new InitialPropsRunner(this.connector, initialProps);
        runner.run(callback);
    }

    runWithFirstComponent(firstComponent: React.ComponentType, callback: (Root: React.FunctionComponent<{}>) => void) {
